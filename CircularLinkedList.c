/*
    Circular LinkedList implementation for integer type data
*/
...
//Structure for the Node of CircularLinkedList
struct node {
    int data;
    struct node* next;
};
...
NODE* createNode () {
    ...
    newNode->next = newNode; //Node is pointing to itself
    printf("Enter Data : ");
...
NODE* getNode(NODE* head, int position) {
    ...
    while (counter != position) {
        list = list->next;
        //if list reached its starting point
        if(list == head) {
...
void insertNode(NODE** addressOfHead, int position) {
    ...
    if(position == getListSize(*addressOfHead)) {
        ...
        NODE* lNode = getNode(*addressOfHead, getListSize(*addressOfHead)); //fetching last node
        newNode->next = cHead;
        *addressOfHead = newNode;
        lNode->next = *addressOfHead; //setting last nodes next to the new head
    }
    else {
...
void deleteNode (NODE** addressOfHead, int pos) {
    ...
    if (pos == 1) {
        *addressOfHead = list->next;
        NODE* lNode = getNode(list, getListSize(list)); //fetching last node
        free(list);
        lNode->next = *addressOfHead; //setting last nodes next to the new head
    } else if (pos == getListSize(list)) {
        NODE* loc = getNode(list, getListSize(list)); //fetching the last node
        loc->next = NULL; //setting last nodes next to null
        NODE* lNode = getNode(list, getListSize(list)); //fetching node before the last node, this become the newLastNode
        lNode->next = list; //setting newLastNode's next to head;
        free(loc);
    } else {
